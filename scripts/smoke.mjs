...
async function testSkipsAndReport(browser) {
  ...
  try {
    ...
    assert.equal(await page.getByRole('button', { name: 'Play 20 Questions' }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Play Story Adventure' }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Play Dungeon Labyrinth' }).count(), 0)
...
async function testDungeon(browser) {
  ...
  try {
    page.on('pageerror', error => { throw error })
    await page.goto(`http://localhost:${port}/`)
    await page.getByRole('button', { name: 'Typing games' }).click()
    assert(await page.getByRole('heading', { name: 'Typing games.' }).isVisible())
    ...
    assert.equal(await page.getByLabel('CHAT MODEL').inputValue(), '~deepseek/deepseek-flash-latest')
    assert.deepEqual(await page.getByLabel('CHAT MODEL').locator('option').evaluateAll(options => options.map(option => option.value)), ['~deepseek/deepseek-flash-latest', 'openai/gpt-oss-120b', '~anthropic/claude-sonnet-latest', 'google/gemini-3.8-flash', '~openai/gpt-sol-latest'])
    await page.getByLabel('CHAT MODEL').selectOption('openai/gpt-oss-120b')
    await page.getByLabel('OPENROUTER API KEY').fill('testing-only-key')
    await page.getByRole('button', { name: 'Save settings' }).click()
    await page.getByRole('status').getByText('Settings saved on this device.').waitFor()
    await page.getByRole('button', { name: 'Typing games' }).click()
    ...
    const initialRoom = Number(await page.locator('.dungeon-grid .visited.current b').innerText())
    assert(await page.getByText('DUNGEON MAP · LEVEL 1').isVisible())
    assert.equal(await page.locator('.dungeon-grid .absent, .dungeon-grid .mapped').count(), 0)
    assert.equal(await page.locator('.dungeon-room').count(), 0)
    assert(await page.locator('.dungeon-grid .dungeon-tile:not(.visited)').first().evaluate(element => getComputedStyle(element).backgroundColor === 'rgba(0, 0, 0, 0)'))
    ...
    await page.route('https://openrouter.ai/api/v1/chat/completions', async route => {
      ...
      if (action.includes('pikk')) content = { corrections: [{ original: 'pikk', replacement: 'pick', explanation: 'Use ck at the end.' }] }
         ...
         const direction = /\b(north|top)\b/.test(action) ? 'north' : /\b(east|right)\b/.test(action) ? 'east' : /\b(south|bottom)\b/.test(action) ? 'south' : /\b(west|left)\b/.test(action) ? 'west' : 'none'
         content = { corrections: [], verb: /go to room \d+/.test(action) ? 'travel' : action.includes('peek') ? 'peek' : action.includes('walk') ? 'move' : action.includes('open the chest') ? 'open' : action.includes('stampy') ? 'ask' : action.includes('look carefully') ? 'inspect' : 'take', direction, item: action.includes('bomb') ? state.floor.find(item => item.name === 'Bomb Kit')?.id ?? '0:bomb-kit' : 'gold', items: action.includes('gold and the bomb kit') ? ['gold', '0:bomb-kit'] : [], name: '', topic: action.includes('open the chest') ? 'chest' : '', reply: '' }
      }
       if (action.includes('look carefully')) await new Promise(resolve => setTimeout(resolve, 450))
    ...
    await input.press('Enter')
     await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(/cannot find/).waitFor()
     await page.waitForFunction(() => document.activeElement?.id === 'dungeon-action')
     assert.equal(await page.locator('.dungeon-inventory li').first().innerText(), 'Empty for now. Pick up anything useful.')
    ...
    const open = await page.locator('.dungeon-choices li').allInnerTexts()
     const match = open.find(choice => choice.startsWith('Walk through the'))?.match(/the (north|east|south|west) door/)
    ...
    await page.getByRole('button', { name: 'Do it' }).click()
     await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(/peek through/i).waitFor()
    assert.equal(await page.locator('.dungeon-grid .visited').count(), 1)
    ...
    await page.locator('.dungeon-grid .visited').nth(1).waitFor()
     ...
     assert(await page.locator('.dungeon-call-time').last().innerText().then(text => /^\d+\.\ds$/.test(text)))
     assert(await page.getByText('A small wooden chest sits against the wall.', { exact: false }).count() >= 1)
     const goldHere = /gold coins glimmer nearby/.test(await page.locator('.dungeon-chat .twenty-message.guide').last().innerText())
     await input.fill('I open the chest')
     await input.press('Enter')
     await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(/open the wooden chest.*bomb kit/i).waitFor()
     await input.fill(goldHere ? 'I pick up the gold and the bomb kit' : 'I pick up the bomb kit')
     await input.press('Enter')
     await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(goldHere ? /gold coins.*bomb kit/ : /pick up the bomb kit/).waitFor()
    ...
    const currentRoom = Number(await page.locator('.dungeon-grid .visited.current b').innerText())
     const entranceTile = page.locator('.dungeon-grid button.visited').filter({ has: page.locator('b').getByText(String(initialRoom), { exact: true }) })
     await entranceTile.hover()
     assert.match(await page.locator('.dungeon-map-tooltip').innerText(), new RegExp(`Room ${initialRoom}:.*Stampy.*door`))
     assert.match(await entranceTile.getAttribute('title'), /Stampy.*door/)
     const unvisitedRoom = Array.from({ length: 25 }, (_, i) => i + 1).find(number => number !== initialRoom && number !== currentRoom)
     await input.fill(`I go to room ${unvisitedRoom}`)
    await input.press('Enter')
    ...
    assert.equal(Number(await page.locator('.dungeon-grid .visited.current b').innerText()), currentRoom)
     await input.fill(`I go to room ${initialRoom}`)
    await input.press('Enter')
    await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(/retrace your steps/).waitFor()
     assert.equal(await page.locator('.dungeon-grid .visited.current b').innerText(), String(initialRoom))
     await input.fill(`I go to room ${initialRoom} then go ${openDirection}`)
    await input.press('Enter')
    ...
    assert.equal(Number(await page.locator('.dungeon-grid .visited.current b').innerText()), currentRoom)
    await page.waitForFunction(() => document.activeElement?.id === 'dungeon-action')
      assert.equal(calls, 10, 'Chained room-and-door navigation should use one spelling-and-action call')
      await input.fill('I look carefully around the room')
      await input.press('Enter')
      assert(await page.locator('.dungeon-chat .twenty-message.player').last().getByText('I look carefully around the room').isVisible())
      await page.locator('.dungeon-chat .twenty-message.guide.thinking').getByText(/thinking/).waitFor()
      await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(/extra gold.*guardian hint/).waitFor()
      assert.equal(await page.locator('.dungeon-chat .twenty-message.guide.thinking').count(), 0)
      assert.match(await page.locator('.dungeon-call-time').last().innerText(), /^\d+\.\ds$/)
     await input.fill(`I go to room ${initialRoom}`)
     await input.press('Enter')
     await page.waitForFunction(number => document.querySelector('.dungeon-grid .visited.current b')?.textContent === String(number), initialRoom)
      await input.fill('I ask Stampy about Indy')
      await input.press('Enter')
      await page.locator('.dungeon-chat .twenty-message.guide').last().getByText(/black cocker spaniel brother/).waitFor()
     if (process.env.DUNGEON_SCREENSHOT) await page.locator('.dungeon-content').screenshot({ path: process.env.DUNGEON_SCREENSHOT })
     await page.reload()
     await page.getByRole('button', { name: 'Settings' }).click()
     assert.equal(await page.getByLabel('CHAT MODEL').inputValue(), 'openai/gpt-oss-120b')
     await page.getByRole('button', { name: 'Typing games' }).click()
     await page.getByRole('button', { name: 'Play Dungeon Labyrinth' }).click()
    assert.equal(await page.locator('.dungeon-grid .visited').count(), 2)
    assert.equal(await page.locator('.dungeon-inventory li').first().innerText(), 'Bomb Kit')
...
async function testTwentyQuestions(browser) {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Back to typing games' }).click()
    assert.equal(await page.locator('.twenty-promo-card').count(), 3)
    await page.getByRole('button', { name: 'Typing test' }).click()
    assert.equal(await page.locator('.twenty-promo-card').count(), 0)
  } finally { await page.close() }
}
